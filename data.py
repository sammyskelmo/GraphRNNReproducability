...
def encode_adj_full(adj):
    ...
    for i in range(adj.shape[0]):
        ...
        adj_slice = adj[i, input_start:input_end]
        # write adj
        adj_output[i, 0:adj_slice.shape[0],
                   0] = adj_slice[::-1]  # put in reverse order
...
class Graph_sequence_sampler_truncate():
    ...
    def sample(self):
        ...
        # here zeros are padded for small graph
        x_batch = np.zeros((self.batch_size, self.n, self.max_prev_node))
        # here zeros are padded for small graph
        y_batch = np.zeros((self.batch_size, self.n, self.max_prev_node))
...
class Graph_sequence_sampler_bfs_permute_truncate_multigraph():
    ...
    def sample(self):
        ...
        # batch, length, length
        # here zeros are padded for small graph
        adj_batch = np.zeros((self.batch_size, self.n, self.n))
        # batch, size, size
        # here zeros are padded for small graph
        adj_norm_batch = np.zeros((self.batch_size, self.n, self.n))
        ...
        for i in range(self.batch_size):
            ...
            feature_degree = np.array(list(G.degree(node_list).values()))[
                :, np.newaxis]
            feature_clustering = np.array(
                list(nx.clustering(G, nodes=node_list).values()))[:, np.newaxis]
            time2 = time.time()
        ...
        return torch.from_numpy(x_batch).float(), torch.from_numpy(y_batch).float(),\
            torch.from_numpy(adj_batch).float(), torch.from_numpy(
                adj_norm_batch).float(), torch.from_numpy(feature_batch).float()
...
def Graph_synthetic(seed):
    ...
    node_distance_l1 = np.ones(
        (node_features.shape[0], node_features.shape[0]))
    node_distance_np = np.zeros(
        (node_features.shape[0], node_features.shape[0]))
    for i in range(node_features.shape[0]):
        for j in range(node_features.shape[0]):
            if i != j:
                ...
                node_distance_np[i, j] = 1 / \
                    np.sum(np.abs(node_features[i] - node_features[j]) ** 2)
...
class GraphDataset_adj(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        sample = {'adj': self.adj, 'adj_norm': self.adj_norm,
...
class GraphDataset_adj_batch(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # in case the self connection already exists
        np.fill_diagonal(adj_raw, 0)

        # sample num_nodes size subgraph
        subgraph_idx = np.random.permutation(adj_raw.shape[0])[
            0:self.num_nodes]
        ...
        if self.has_feature:
            ...
            features = np.zeros(
                (self.num_nodes, list(dictionary.values())[0].shape[0]))
        ...
        sample = {'adj': adj, 'adj_norm': adj_norm,
                  'features': features, 'adj_raw': adj_raw}
        return sample
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.hops):
            ...
            for j, idx in enumerate(idx_list):
                ...
                adj_list[start_idx:start_idx +
                         incre_idx] = adj_list_new[:incre_idx]
            index = torch.from_numpy(adj_list).long()
            adj_list_emb = self.embedding[index]
