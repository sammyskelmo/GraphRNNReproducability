...
def degree_stats(graph_ref_list, graph_pred_list, is_parallel=False):
    ...
    if is_parallel:
        ...
        for i in range(len(graph_pred_list_remove_empty)):
            degree_temp = np.array(nx.degree_histogram(
                graph_pred_list_remove_empty[i]))
            sample_pred.append(degree_temp)
    print(len(sample_ref), len(sample_pred))
    mmd_dist = mmd.compute_mmd(
        sample_ref, sample_pred, kernel=mmd.gaussian_emd)
...
def clustering_stats(graph_ref_list, graph_pred_list, bins=100, is_parallel=False):
    ...
    if is_parallel:
        ...
        for i in range(len(graph_pred_list_remove_empty)):
            clustering_coeffs_list = list(nx.clustering(
                graph_pred_list_remove_empty[i]).values())
            hist, _ = np.histogram(
                clustering_coeffs_list, bins=bins, range=(0.0, 1.0), density=False)
