...
def n_community(c_sizes, p_inter=0.01):
    graphs = [nx.gnp_random_graph(c_sizes[i], 0.7, seed=i)
...
def save_prediction_histogram(y_pred_data, fname_pred, max_num_node, bin_n=20):
    ...
    imsave(fname=fname_pred, arr=output_pred, origin='upper',
           cmap='Greys_r', vmin=0.0, vmax=3.0 / bin_n)
...
def draw_graph(G, prefix='test'):
    ...
    nx.draw_networkx(G, with_labels=True, node_size=35,
    ...
    plt.loglog(np.arange(len(G_deg))[G_deg > 0],
               G_deg[G_deg > 0], 'r', linewidth=2)
...
if __name__ == '__main__':
    ...
    for i in range(0, 160, 16):
        draw_graph_list(graphs[i:i+16], 4, 4,
                        fname='figures/community4_' + str(i))
